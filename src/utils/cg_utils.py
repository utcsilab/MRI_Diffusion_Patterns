...
import torch
from src.utils.helpers import MRI_forward_nomask, ifft
...
def get_Aop_fun(mask, s_maps):
    ...
    def Aop_fun(x):
        Ax = mask * MRI_forward_nomask(x, s_maps) #[N, C, H, W] complex, undersampled measurements
        
        return torch.sum(ifft(Ax) * torch.conj(s_maps), axis=1) #[N, H, W] complex
...
def get_cg_rhs(mask, s_maps, FSx, l2lam, x_init):
    ...
    A_conj_b = torch.sum(ifft(b) * torch.conj(s_maps), axis=1) #[N, H, W] complex
