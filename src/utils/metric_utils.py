...
def get_ssim(x_hat, x):
    """
    Calculates SSIM(x_hat, x) over individual samples in a batch.
    
    Args:
        x_hat (torch.Tensor): Predicted images. Shape (N, 1, H, W).
        x (torch.Tensor): Ground truth image. Shape (N, 1, H, W).
    
    Returns:
        np.array: SSIM values for each image in the batch. Shape [N].
    """
    ssim_vals = []
...
def get_psnr(x_hat, x):
    """
    Calculates PSNR(x_hat, x) over individual samples in a batch.
    ...
    Returns:
        np.array: PSNR values for each image in the batch. Shape [N].
    """
    psnr_vals = []
    for i in range(x_hat.shape[0]):
        im1 = x_hat[i,0].detach().cpu().numpy()
...
def get_all_metrics(x_hat, x):
    ...
    Returns a dictionary of metrics valkues stored as np arrays.
    ...
    Returns:
        dict: Dictionary of metrics values. Metric values are stored as np arrays with shape [N].
    """
